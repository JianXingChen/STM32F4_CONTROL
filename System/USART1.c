...
float ChangeToPosition(uint16_t Data);
...
void DMA2_Stream5_IRQHandler(void)
{
    if(DMA_GetITStatus(DMA2_Stream5, DMA_IT_TCIF5))
    {
		...
		Ch[1]=ChangeToPosition(RC_Ctl.rc.ch1);
		Ch[2]=ChangeToPosition(RC_Ctl.rc.ch2);
		Ch[3]=ChangeToPosition(RC_Ctl.rc.ch3);
		
}

float ChangeToPosition(uint16_t Data)     //转换为竖直方向角度
{
	float Position=0;
	Position=Data*0.0447-64.27;
