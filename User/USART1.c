...
float ChangeToPosition_Pitch(uint16_t Data);
float ChangeToPosition_Yaw(uint16_t Data);
...
{
		float yaw_temp;//此变量一定要使用，为了防止计算中间时没有矫正过的变量被调用
    ...
    if(DMA_GetITStatus(DMA2_Stream5, DMA_IT_TCIF5))
    {
		...
		Pitch_Hold_Info.angle_target = (1024 - RC_Ctl.rc.ch3 ) * 0.03f;//pitch 遥控器输入转换成角度
		
		yaw_temp = yaw_Hold_Info.angle_target + ( 1024 - RC_Ctl.rc.ch2 ) * 0.0045f;
		...
		yaw_Hold_Info.angle_target  = yaw_temp;
		
//		Ch[0]=ChangeToPosition_Yaw(RC_Ctl.rc.ch0);
//		Ch[1]=RC_Ctl.rc.ch1;
//		Ch[2]=RC_Ctl.rc.ch2;
//		Ch[3]=ChangeToPosition_Pitch(RC_Ctl.rc.ch3);
		
}

float ChangeToPosition_Pitch(uint16_t Data)     //转换为竖直方向角度  最高处-1，最低处44.5
{
	float Position=0;
	Position = Data*0.034 - 34.909;
	return (short)Position;
}

float ChangeToPosition_Yaw(uint16_t Data)     //转换为水平方向角度，-90——90度
{
	float Position=0;
	Position=Data*0.136-139.636;
	return (short)Position;
}
